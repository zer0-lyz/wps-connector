...
function statusForError(error) {
  ...
  if (code === "SESSION_HOST_MISMATCH" || code === "SESSION_BINDING_MISMATCH" || code === "BINDING_MISMATCH" || code === "SESSION_BINDING_REQUIRED" || code === "SESSION_AMBIGUOUS" || code === "SESSION_OFFLINE" || code === "SESSION_NOT_ROUTABLE" || code.endsWith("_REFUSED")) return 409;
...
function sessionAgeMs(session) { return Math.max(0, Date.now() - sessionLastSeenMs(session)); }
function sessionIsRoutable(session) { return session?.status === "online" && !session.unresponsiveSince && sessionAgeMs(session) <= sessionFreshMs; }
...
function listSessions(input = {}) {
  ...
  const onlyRoutable = queryBool(input.onlyRoutable, false);
  ...
  if (onlyRoutable) items = items.filter((session) => sessionIsRoutable(session));
...
async function connectionStatus(input = {}) {
  pruneOfflineSessions();
  const { host: _hostFilter, sessionId: _sessionFilter, onlyOnline: _onlyOnline, onlyRoutable: _onlyRoutable, onlyBound: _onlyBound, ...bindingInput } = input;
  ...
  const routable = filtered.filter((session) => session.routable);
  const recommendable = candidates.filter((session) => session.routable && (!input.onlyBound || session.bound));
  ...
  if (online.length && !routable.length) issues.push({ code: "NO_ROUTABLE_SESSIONS", message: "WPS sessions are registered, but none are currently polling for commands. Activate the target WPS window or pane before running tools.", details: { onlineCount: online.length, staleSessions: online.map((session) => ({ sessionId: session.sessionId, host: session.host, documentName: session.documentName, lastSeenAt: session.lastSeenAt, ageMs: session.ageMs })) } });
  if (!input.sessionId && recommendable.length > 1) issues.push({ code: "SESSION_AMBIGUOUS", message: "Multiple routable WPS sessions match the requested selector. Pick one sessionId before running Writer/Spreadsheet tools.", details: { matchCount: recommendable.length, candidates: recommendable.map((session) => ({ sessionId: session.sessionId, host: session.host, documentName: session.documentName, documentKey: session.documentKey, binding: session.binding || null })) } });
  ...
  if (input.sessionId) {
    ...
    if (exact && exact.status === "online" && !exact.routable) issues.push({ code: "SESSION_NOT_ROUTABLE", message: "The requested session is registered but not currently polling for commands.", details: { sessionId: input.sessionId, lastSeenAt: exact.lastSeenAt, ageMs: exact.ageMs } });
  ...
  if (issues.some((issue) => issue.code === "SESSION_AMBIGUOUS")) nextActions.push("Use wps.list_sessions with onlyOnline:true and host, then pass the selected routable sessionId explicitly in every tool call.");
  if (issues.some((issue) => issue.code === "NO_ROUTABLE_SESSIONS" || issue.code === "SESSION_NOT_ROUTABLE")) nextActions.push("Bring the target WPS document window to the front, or reopen its WPS Connector pane, then retry.");
  ...
  return {
    ...
    filters: { onlyOnline: Boolean(input.onlyOnline), onlyRoutable: Boolean(input.onlyRoutable), onlyBound: Boolean(input.onlyBound), host: input.host || "", sessionId: input.sessionId || "" },
    counts: { total: filtered.length, online: online.length, routable: routable.length, onlineBound: onlineBound.length, matched: candidates.length },
...
async function sendListSessions(req, res, url) {
  ...
  return sendJson(res, 200, { ok: true, sessions: sessionsList, count: sessionsList.length, filters: { onlyOnline: queryBool(input.onlyOnline, false), onlyRoutable: queryBool(input.onlyRoutable, false), includeOffline: queryBool(input.includeOffline, false), sessionId: input.sessionId || "", documentKey: input.documentKey || "", host: input.host || "" }, agentUsage: bridgeHelp() });
}
